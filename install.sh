...
echo -e "${YELLOW}  1. Set station name: sudo nano /etc/phenocam/settings.txt${NC}"
...
echo -e "${YELLOW}     sudo nano /etc/phenocam/ftp_credentials.txt${NC}"
echo ""
echo -e "${YELLOW}  SFTP (recommended for security — SSH key, no password over network):${NC}"
echo -e "${YELLOW}     1. Send the public key above to your SFTP server administrator${NC}"
echo -e "${YELLOW}     2. sudo nano /etc/phenocam/server.txt  (add server hostname)${NC}"
echo -e "${YELLOW}     3. sudo ssh-keyscan -H <hostname> >> /etc/phenocam/known_hosts${NC}"
echo -e "${YELLOW}     4. Edit line 8 of settings.txt (SFTP_USER)${NC}"
echo -e "${YELLOW}     5. Edit remote_dir in upload_sftp.sh (TBD_REMOTE_DIR)${NC}"
