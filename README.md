...
9. Align SSH key permissions for the `phenocam` service user
10. Install log rotation for `phenocam.log`, systemd units and udev rules
11. Run a first capture + upload cycle as a health check
12. Print installation status and next steps
...
```bash
ssh-keyscan -H <hostname_or_ip> | sudo tee -a /etc/phenocam/known_hosts >/dev/null
```
...
#### 6. Recommended manual test before enabling production uploads

```bash
sudo -u phenocam sftp \
  -i /etc/phenocam/keys/phenocam_key \
  -oBatchMode=yes \
  -oStrictHostKeyChecking=yes \
  -oUserKnownHostsFile=/etc/phenocam/known_hosts \
  <SFTP_USER>@<hostname_or_ip>
```

If this test fails, do not enable SFTP in the automatic upload flow yet.

#### Example: ICOS SFTP activation

For an ICOS-compatible setup, a typical local configuration is:

- `/etc/phenocam/server.txt` → `icos01.uantwerpen.be`
- `/etc/phenocam/settings.txt` line 8 (`SFTP_USER`) → value provided by the server administrator
- `/etc/phenocam/settings.txt` line 14 (`REMOTE_LAYOUT`) → `icos`
- `/etc/phenocam/known_hosts` → populated with `ssh-keyscan`
...
In the current implementation, SFTP is attempted before FTP. If SFTP fails in a given cycle, FTP may not be reached in that same cycle and files remain queued for retry.
