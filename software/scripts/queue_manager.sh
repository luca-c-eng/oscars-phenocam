...
#   4. STOP     — if SD usage >= SD_MAX_USED_PCT (capture skipped)
#
# Return codes:
#   0  — success
...
enqueue_pair() {
  ...
  # Check SD threshold first — if SD is full, stop capturing.
  local sd_used
  ...
  # rename(2) is atomic on Linux — prevents the uploader from reading
